...
data = pd.read_csv(r'C:\Cricket Adda\T20 Data\IPL Ball Tracking Data\ipl_all_matches.csv')
df = pd.DataFrame(data, columns = ['batter_name', 'batter', 'bowler_name', 'bowl_style', 'length_type', 'year', 'match_id', 'match_inn', 'over_ball', 'over_num', 'speed', 'catcher', 'total_extras', 'runs', 'bowler_extras', 'otw', 'length'])
df = df.rename(columns={'catcher': 'player_dismissed'})
df = df[df['year'] >= 2018]
df = df[df['speed'] > 105]
df = df[df['over_num'] < 21]
name = 'Suresh Raina'
df = df[df['batter_name']==name]
df1 = df[df['length_type'] == 'Full toss']
index = df1.index
balls_ft = len(index)
